...
I use [[https://www.gnu.org/software/emacs/manual/html_node/eglot/index.html][eglot]] as an LSP client to interface with [[https://clangd.llvm.org/][clangd]] and [[https://github.com/palantir/python-language-server][python-language-server]]. Due to
performance concerns, the client is not automatically loaded when a code file is
opened. The =SPC l= keystroke turns LSP on. In =prog-mode=, commands under =SPC
m= run LSP functions I commonly use. [[https://www.gnu.org/software/emacs/manual/html_node/flymake/index.html][Flymake]] is use for syntax highlighting.
...
- [[https://www.jamescherti.com/essential-emacs-packages/][Must-have Emacs packages]] by James Cherti
